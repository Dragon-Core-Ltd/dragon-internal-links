...
// Variables: $orphans, $low_outbound, $current_tab
?>
                ...
                <tbody>
                    <?php foreach ( $orphans as $post ) : ?>
                        <tr>
                            ...
                            <td class="dil-center"><?php echo esc_html( $post['outbound_count'] ); ?></td>
                            <td>
                                <?php
                                $score = (float) $post['orphan_score'];
                                $priority_class = $score > 5 ? 'high' : ( $score > 2 ? 'medium' : 'low' );
                                ?>
                                <span class="dil-priority dil-priority-<?php echo esc_attr( $priority_class ); ?>">
                                    <?php echo esc_html( ucfirst( $priority_class ) ); ?>
                                </span>
                            </td>
                            <td>
                                <a href="<?php echo esc_url( get_permalink( $post['post_id'] ) ); ?>" target="_blank" class="button button-small">
    ...
    <div class="dil-card" style="margin-top: 20px;">
        ...
        <?php else : ?>
            <table class="dil-table widefat">
                ...
                <tbody>
                    <?php foreach ( array_slice( $low_outbound, 0, 25 ) as $post ) : ?>
                        <tr>
                            <td>
                                <a href="<?php echo esc_url( get_edit_post_link( $post['post_id'] ) ); ?>">
                                    <?php echo esc_html( $post['post_title'] ); ?>
                            ...
                            <td class="dil-center">
                                <span class="dil-badge dil-badge-warning"><?php echo esc_html( $post['outbound_count'] ); ?></span>
                            </td>
                            <td class="dil-center"><?php echo esc_html( $post['inbound_count'] ); ?></td>
                            <td>
                                <a href="<?php echo esc_url( get_edit_post_link( $post['post_id'] ) ); ?>" class="button button-small button-primary">
